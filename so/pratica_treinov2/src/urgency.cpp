...
static pthread_cond_t *patients_done;
...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd)
{
   ...
   patients_done = new pthread_cond_t[np];

   for (uint32_t i = 0; i < np; i++)
   {
      patients_mutex[i] = PTHREAD_MUTEX_INITIALIZER;
      patients_done[i] = PTHREAD_COND_INITIALIZER;
   }
...
void *doctor_life(void *arg)
{
   while (!done)
   {
      ...
      printf("\e[32;01mDoctor: patient %u treated\e[0m\n", patient);
      hd->all_patients[patient].done = 1;
      cond_broadcast(&patients_done[patient]);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   check_valid_name(hd->all_patients[id].name);
   while (not hd->all_patients[id].done)
      cond_wait(&patients_done[id], &patients_mutex[id]);
...
{
   int number = *(int *)(id);
   patient_goto_urgency(number);
   //nurse_iteration();  // to be deleted in concurrent version
   //doctor_iteration(); // to be deleted in concurrent version
   patient_wait_end_of_consultation(number);
   memset(&(hd->all_patients[number]), 0, sizeof(Patient)); // patient finished
   return NULL;
}
...
int main(int argc, char *argv[])
{
   ...
   for (uint32_t i = 0; i < npatients; i++)
   {
      ...
      *id = i;
      thread_create(&patients[i], NULL, patient_life, id);
   }
