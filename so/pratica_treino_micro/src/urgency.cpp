...
static pthread_t* patients;
static pthread_t* nurses;
static pthread_t* doctors;

static pthread_mutex_t* patientsaccess;
static pthread_cond_t* patientsready;
...
void init_simulation(uint32_t np, uint32_t nd, uint32_t nn)
{
   ...
   nurses = new pthread_t[nn];
   doctors = new pthread_t[nd];
   patients = new pthread_t[np];

   patientsaccess = new pthread_mutex_t[np];
   patientsready = new pthread_cond_t[np];

   for(uint32_t i=0; i< np; i++){
   		patientsaccess[i] = PTHREAD_MUTEX_INITIALIZER;
   		patientsready[i] = PTHREAD_COND_INITIALIZER; 
   }
...
{
	mutex_lock(&patientsaccess[id]);
   check_valid_name(hd->all_patients[id].name);
   while(! hd->all_patients[id].done){
   	cond_wait(&patientsready[id], &patientsaccess[id]);
   }
   printf("\e[30;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);

   	mutex_unlock(&patientsaccess[id]);
...
int main(int argc, char *argv[])
{
   ...
   init_simulation(npatients, ndoctors, nnurses);
	...
	for(uint32_t i=0; i<nnurses; i++){
		thread_create(& nurses[i], NULL, nurselife, NULL);
	}

	for(uint32_t i=0; i<ndoctors; i++){
			thread_create(& doctors[i], NULL, doctorlife, NULL);
	}
   ...
   for(uint32_t i = 0; i < npatients; i++)
   {
      ...
      *id = i;
      thread_create(&patients[i], NULL, patient_life, (void*)id); 
    }
